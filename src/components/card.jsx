import { useRef } from "react";
import { Link } from "react-router-dom";

export default function Card({
  title,
  desc,
  tags,
  image,
  url,
  meta,
  internal,
  label,
}) {
  const cardRef = useRef(null);

  const handleMouseMove = (e) => {
    const card = cardRef.current;
    if (!card) return;
    const rect = card.getBoundingClientRect();
    card.style.setProperty("--x", `${e.clientX - rect.left}px`);
    card.style.setProperty("--y", `${e.clientY - rect.top}px`);
  };

  const hasLink = url && url !== "#";
  const linkLabel = label || (internal ? "Baca selengkapnya" : "Lihat project");

  // pilih wrapper: Link (internal), a (external), atau div
  let Wrapper = "div";
  let wrapperProps = {};

  if (hasLink && internal) {
    Wrapper = Link;
    wrapperProps = { to: url };
  } else if (hasLink) {
    Wrapper = "a";
    wrapperProps = { href: url, target: "_blank", rel: "noopener noreferrer" };
  }

  return (
    <Wrapper
      className={`project-card ${!hasLink ? "no-link" : ""}`}
      ref={cardRef}
      onMouseMove={handleMouseMove}
      {...wrapperProps}
    >
      <div className="pc-glow"></div>
      <div className="pc-thumb">{image && <img src={image} alt={title} />}</div>
      <div className="pc-body">
        <div className="wtitle">{title}</div>
        {meta && <div className="wmeta">{meta}</div>}
        {desc && <div className="wdesc">{desc}</div>}
        {tags && (
          <div className="wtags">
            {tags.map((tag) => (
              <span className="tag" key={tag}>
                {tag}
              </span>
            ))}
          </div>
        )}
      </div>
      {hasLink && (
        <div className="pc-link">
          {linkLabel} <span className="arrow">↗</span>
        </div>
      )}
    </Wrapper>
  );
}
